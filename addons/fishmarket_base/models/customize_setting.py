...
class AppThemeConfigSettings(models.TransientModel):
    ...
    now_date = fields.Date(string='現在系統日期',default=lambda self:fields.Datetime.now(),readonly=True)

    @api.depends('working_date')
    def setting_date(self):
        for line in self:
            line.now_date = line.working_date
    ...
    def set_default_all(self):
        self.ensure_one()
        ir_config = self.env['ir.config_parameter']
        ir_config.set_param("now_date", self.working_date or "")
